...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("GYMKHΛИΛ MØD ® | Kuyruğa Ekle")
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("GYMKHΛИΛ MØD ® | Oynatılan Şarkı")
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("GYMKHΛИΛ MØD ® | Kuyruk Bitti!")
  .setDescription(`<a:hypesquad1:750076071721828452> **| Kuyruktaki tüm müzikler oynatıldı. GYMKHΛИΛ MØD ® kanaldan ayrılıyor.\n\nGYMKHΛИΛ MØD ® Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
