...
.setThumbnail()
.addField(`GYMKHΛИΛ MØD ® | Teşekkürler`, `**Selamlar, Ben Lord Creative (GYMKHΛИΛ MØD ®'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`GYMKHΛИΛ MØD ® | Prefix`, `**GYMKHΛИΛ MØD ® Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`GYMKHΛИΛ MØD ® | Nasıl Kullanılır?`, `**GYMKHΛИΛ MØD ® botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`GYMKHΛИΛ MØD ® | Linkler`, `**Sohbet Kanalına gm!davet Yazmanız Yeterlidir**`)
.setFooter(`GYMKHΛИΛ MØD ® | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@815959630718828595>`) return msg.channel.send(`Yapımcım <@774591026940739585> `);
});
...
.setDescription(` <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("GYMKHΛИΛ MØD ® Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`GYMKHΛИΛ MØD ® Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
